...
from reproduction.initialization import verify_initialization
...
from access_curriculum import schedule
...
def restore_training_curve(path: Path, curve: list[dict[str, Any]], step: int) -> None:
    """Restore the authoritative checkpoint history before continuing a retry."""
    steps = [row["step"] for row in curve]
    if steps != sorted(set(steps)) or any(s < 1 or s > step for s in steps):
        raise ValueError("recovery curve has invalid or duplicate steps")
...
class ResolvedConfig:
    ...
    router: str
    topology: str
    access_stages: tuple[tuple[int, int, int, int], ...]
...
def resolved_config(arm_name: str) -> ResolvedConfig:
    ...
    return ResolvedConfig(
        ...
        evidence_tier="small_wikitext_plus_recall",
        arm=arm.identifier,
        ...
        access_stages=schedule.stages(arm_name, "wikitext"),
    )
...
def save_recovery(
    ...
    *,
    ...
    atomic_torch_save(
        path,
        {
            "schema": "access-curriculum-recovery-v1",
            "step": step,
...
def load_recovery(
    ...
    *,
    ...
    if payload.get("schema") != "access-curriculum-recovery-v1":
...
def train(args: argparse.Namespace) -> None:
    if (args.reads, args.writes) != (8, 8):
        raise ValueError("terminal access must be explicitly R=W=8")
    ...
    if arm.has_sdm:
        verify_initialization(model, task="wikitext", layout=arm.profile.layout, router=arm.router)
    ...
    if args.resume is not None:
        restore_training_curve(metric_path, curve, start_step)
    with metric_path.open(
        ...
        for step_index in range(start_step, TOTAL_STEPS):
            step = step_index + 1
            schedule.apply(model, args.arm, step, "wikitext")
    ...
    atomic_torch_save(
        terminal_checkpoint,
        {
            "schema": "access-curriculum-terminal-model-v1",
            "arm": arm.identifier,
            "config": asdict(config),
            "manifest_sha256": manifest_sha256,
            "model": model.state_dict(),
    ...
    atomic_json(output / "coverage.json", coverage)
    result = {
        "schema": "access-curriculum-wikitext103-result-v1",

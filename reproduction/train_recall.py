...
from reproduction.recall_spec import learning_rate
...
def build_model(arm: str):
    ...
    model.bfloat16()
    if ARMS[arm].has_sdm:
        verify_initialization(model, task="recall", layout=ARMS[arm].profile.layout,
...
def train(args) -> None:
    ...
    with (output / "training_curve.jsonl").open("a") as curve:
        for step_index in range(start_step, end):
            step = step_index + 1
            access = schedule.apply(model, args.arm, step, "recall")
            batch, labels, _ = data.train_batch(step_index)
            model.train()
            optimizer.zero_grad(set_to_none=True)
            lr = learning_rate(args.arm, step)
            for group in optimizer.param_groups:
                group["lr"] = lr
            loss_total = 0.0
            # Preserve the recorded single full batch: BF16 accumulation depends on batching.
            micro = BATCH_SIZE
            ...
            optimizer.step()
            row = {"step": step, "train_query_loss": loss_total, "learning_rate": lr, "gradient_norm": float(grad), "access": access}
    ...
    atomic_torch_save(checkpoint, {"schema": "access-curriculum-recall-checkpoint-v1",
    ...
    result = {"schema": "access-curriculum-recall-result-v1", "arm": args.arm,
